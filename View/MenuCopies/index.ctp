<div class="row">
    <div class="col-xs-12">
        <h1 class="page-header">店舗管理　＞　メニューコピー</h1>
        <?php echo $this->Flash->render(); ?>
    </div>
</div>

<?php 
echo $this->Form->create('MenuCopy', array(
    'class' => 'form',
    'action' => 'create'
));
?>
<div class="row">
    <div class="col-xs-6"><h4>メニューのコピー元を選択してください。</h4></div>
    <div class="col-xs-6"><h4>メニューのコピー先を選択してください。</h4></div>
</div>
<?php foreach ($data as $key => $value) : ?>
<div class="row">
    <div class="col-xs-6">
        <pre><?php echo $value['Group']['name'];
...
<script>
    $(function() {
        ...
        $('#confirm-modal #btn_confirm').click(function() {
            $("#MenuCopyCreateForm").submit();
        });

        $('.copy-history').click(function(){
            $.ajax({
                url: URL+'menuCopies/history/',
                method: 'POST',
                data: {},
                dataType: 'html',
