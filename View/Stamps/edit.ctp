...
echo $this->Form->create('StampSetting',
    array('id' => 'edit_stamp', 'class' => 'form-horizontal', 'type' => 'file'));
?>
<style type="text/css">
    .select_img1 {
        border-radius: 13px;
    ...
    *</style>
    ...
    $app_launch        = array();
    //$share_via_sns = array();
    //$app_introductions = array();
    $app_shop_visits   = array();

    for ($j = 0; $j <= 10; $j++) {
        $app_installations[$j] = $j.'個';
        $app_launch[$j]        = $j.'個';
        //$share_via_sns[$j] = $j . '個';
        //$app_introductions[$j] = $j . '個';
        $app_shop_visits[$j]   = $j.'個';
    }
    ?>
...
//                                array(
//                                'type' => 'select',
//                                'options' => $app_introductions,
//                                'class' => 'form-control icon_select',
//                                'label' => false,
//                                'name' => 'app_introduction',
//                                'id' => 'app_introduction',
//                                'value' => $app_introduction_val,
//                            ));
                    ?>
                        </td>
                    </tr>-->
                        ...
                        <td>
                            ...
                            <div class="panel-body select_img1" style="cursor:pointer;" id="add_stemp_img">
                                ...
                                <div id='<?php echo $id; ?>' <?php echo $hide ?> class="select_img show_img ui-state-default profile-pic">
                                    ...
                                    <div id='center_word'>
                                        <?php echo $this->Html->image('/uploads/stamps/'.$benefit_image,
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="panel-body disable_border">
                                <p>発行から
                                    <input class="change_width_input" type="number" value="<?php echo $valid_date_val ?>" name="valid_date" max="60" min="10" onkeypress="return false">
                                    日間</p>
                            </div>
                        </td>
                    </tr>
...
<input type="hidden" id="stamp_id" name="stamp_id" value="<?php echo $id ?>" />
...
<script type="text/javascript" >
    ...
    $(function () {
        ...
        $("body").on("click", "#btn_confirm_save", function (e) {
            e.preventDefault();
            var data = $("#edit_stamp").serialize();
            $.ajax({
                url: "<?php echo Router::url(array('controller' => 'stamps', 'action' => 'edit')); ?>",
                ...
                complete: function () {
                    ...
                    unsaved= false;
                    window.location.replace(URL + 'users/view/app-stamp');
                }
            });
        });
        //On Change image name send to server and store name in hidden
        $("form#edit_stamp").on("change", ".upload", function (e) {
            e.preventDefault();
            upload_photo();
        });
